...
class Dataset_Custom(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_y_mark = self.data_stamp[r_begin:r_end]
        if self.ctx_len > 0:
            ctx_start = max(0, s_end - self.ctx_len)
            ctx_hist = self.data_x[ctx_start:s_end]  # [≤ctx_len, C]

            # 左侧补 0（或其它常数 / nan）
            if ctx_hist.shape[0] < self.ctx_len:
                pad = np.zeros((self.ctx_len - ctx_hist.shape[0],
                                ctx_hist.shape[1]), dtype=ctx_hist.dtype)
                ctx_hist = np.concatenate([pad, ctx_hist], axis=0)  # [ctx_len, C]

            return seq_x, seq_y, seq_x_mark, seq_y_mark, ctx_hist
        else:
            return seq_x, seq_y, seq_x_mark, seq_y_mark
